...
func ToEthState(s *channel.State) EthChannelState {
	...
	for i, backendID := range s.Allocation.Backends {
		...
		case BackendID: // Solana backend ID
			assets[i] = assetToSolanaAsset(s.Allocation.Assets[i])
...
func assetToSolanaAsset(asset channel.Asset) ChannelAsset {
	var assetBytes []byte
	var err error

	switch v := asset.(type) {
	case *SolanaCrossAsset:
		assetBytes, err = v.MarshalBinary()
	default:
		log.Panicf("expected asset of type Solana or MultiAsset, but got: %T", asset)
	}
...
func ToEthParams(params *channel.Params) (ChannelParams, error) {
	...
	if params.App != nil && !channel.IsNoApp(params.App) {
		return ChannelParams{}, errors.New("app is not supported in Solana backend")
	}
